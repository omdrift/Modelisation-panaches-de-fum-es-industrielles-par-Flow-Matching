...
def load_vqvae(checkpoint_path, config_path):
    ...
    vqvae = build_vqvae(vqvae_params)
...
def process_frame(fg, alpha, background, vqvae, device, crop_size=180):
    ...
    # Reconstruct foreground with VQ-VAE
    with torch.no_grad():
        output = vqvae(fg_tensor)
...
def visualize_comparison(video_path, checkpoint_path, config_path, output_path, num_frames=5):
    ...
    vqvae = load_vqvae(checkpoint_path, config_path).to(device)
    ...
    for i, (fg, alpha, original) in enumerate(zip(foregrounds, alphas, originals)):
        ...
        fg_viz, recon_fg, comp_orig, comp_recon = process_frame(fg, alpha, background, vqvae, device)
        results.append({
            'original': original,
            'fg_extracted': fg_viz,
    ...
    for i, result in enumerate(results):
        ...
        axes[i][2].set_title('VQ-VAE Reconstructed FG', fontsize=10)
        axes[i][2].axis('off')
        
        # VQ-VAE + BG Correction
        axes[i][3].imshow(cv2.cvtColor((result['composite_original'] * 255).astype(np.uint8), cv2.COLOR_BGR2RGB))
        axes[i][3].set_title('VQ-VAE + BG Overwrite', fontsize=10)
        axes[i][3].axis('off')
...
def main():
    parser = argparse.ArgumentParser(description="Visualize VQ-VAE reconstruction on video")
    parser.add_argument("--video", type=str, required=True, help="Path to input video")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to VQ-VAE checkpoint")
    parser.add_argument("--config", type=str, default="configs/config_vqvae.yaml", 
                        help="Path to config file")
    parser.add_argument("--output", type=str, default="vqvae_reconstruction_comparison.png",
